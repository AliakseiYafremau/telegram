...
class TelegramBot:
    ...
    def introduction(self, message):
        ...
        @self.bot.callback_query_handler(func=lambda call: call.data in ['diary'])
        def call_handler(call):
            print('[INFO] Running of data handler in Introduction')
            if call.data == 'diary':
                self.user_page(message)
            else:
                self.lesson_page(message)
    ...
    def user_page(self, message):
        ...
        try:
            ...
            self.cursor.execute("CREATE TABLE IF NOT EXISTS user_{0}("
                                "name text,"
                                "grades text)".format(message.from_user.id))
            print('[INFO] Creating of table of user')
            lessons = info('name', 'user_{0}'.format(message.from_user.id))
            keyboard = []
        ...
        @self.bot.callback_query_handler(func=lambda call: (call.data in ['create', 'return_to_main']) or call.data[:3]=='to_')
        def handler(call):
            print('[INFO] Running of data handler in user_page')
    ...
    def request_of_new_lesson(self, message):
        ...
        self.bot.send_message(message.chat.id, 'Введите название урока')
        self.bot.register_next_step_handler(message, self.create_new_lesson)

    def create_new_lesson(self, message):
        print('[INFO] Attempt to create a new lesson')
        try:
            self.database = sqlite3.connect('database.db')
            self.cursor = self.database.cursor()
            self.cursor.execute("SELECT COUNT(*) FROM user_{0} WHERE name= '{1}'".format(message.from_user.id, message.text))
            result = self.cursor.fetchone()
            if result[0] > 0:
                self.bot.send_message(message.chat.id, 'Урок с таким именем уже существует')
            else:
                self.cursor.execute("INSERT INTO user_{0} VALUES {1}".format(message.from_user.id, (message.text, '')))
                self.bot.send_message(message.chat.id, 'Урок  c именем {0} был создан'.format(message.text))
    ...
    def lesson_page(self, message, lesson):
        print('[INFO] Lesson_page')
        try:
            self.database = sqlite3.connect('database.db')
            self.cursor = self.database.cursor()
            self.lessons, self.i = found(lesson, message.from_user.id)
            ...
            keyboard.append([button_2])
            button_3 = types.InlineKeyboardButton('Вернуться', callback_data='return_to_user_page')
        ...
        @self.bot.callback_query_handler(func=lambda call: call.data in ['create_grade', 'delete_grade', 'return_to_user_page'])
        def handler(call):
            if call.data == 'create_grade':
                self.bot.send_message(message.chat.id, 'Напишите какую оценку вы хотите добавить')
                self.bot.register_next_step_handler(message, self.create_grade, lesson=self.lessons[self.i][0])
            elif call.data == 'delete_grade':
                self.bot.send_message(message.chat.id, 'Напишите какую оценку вы хотите удалить')
                self.bot.register_next_step_handler(message, self.delete_grade, lesson=self.lessons[self.i][0])
    ...
    def create_grade(self, message, lesson):
        try:
            ...
            cursor.execute("UPDATE user_{0} SET grades='{1}' WHERE name='{2}'"
                           .format(message.from_user.id, helper.add_number(old, message.text), lesson))
        finally:
            database.commit()
            database.close()
    ...
    def delete_grade(self, message, lesson):
        try:
            ...
            if new_value != -1:
                cursor.execute("UPDATE user_{0} SET grades='{1}' WHERE name='{2}'"
                               .format(message.from_user.id, new_value, lesson))
            else:
                self.bot.send_message(message.chat.id, 'Такой оценки не существует')
        finally:
            database.commit()
            database.close()
        self.lesson_page(message, lesson)
